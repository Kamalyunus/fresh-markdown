...
row readers for parquet, CSV and JSONL, the top-of-hour snapshot, the response."""
import json
import os
...
def snapshot_rows(records):
    """Snapshot records in the engine's names (the discount a fraction), each
    keyed by its shelf-hour; an unkeyable record keeps `key` None so it costs
    itself a response line, never the batch."""
    rows = []
    for r in records:
