...
| `data/` | `flc.parquet`, the day's extract: the trailing 45 days of the hourly table, overwritten every morning (a trailing 30-day rate needs 30 days of rows, and the previous episode's rate reaches a little further) | `download_flc.py` |
...
| `feed/` | yesterday's hourly feed, `<day>.parquet`, if you deliver it as a file for the owner's learning lane; the morning commands do not read it | you, nightly |
