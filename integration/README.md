...
| When | Command | In | Out |
| --- | --- | --- | --- |
| Every clock hour | `python3 price_hour.py --snapshot snapshots/<day>T<hh>.parquet --workers 0 --out decisions/<day>T<hh>.csv --report reports/hours/<day>T<hh>.json` | the shelf at the top of the hour: the twelve request fields (below) | one row per snapshot row, `sku_id, fc, date, hour_of_day, episode_id` echoed: the discount to apply as a percent and as a price, or `rejected` with the reason; the hour's counts |
| Every morning, first | `python3 download_flc.py` | the warehouse: the trailing 45 days of the hourly table through yesterday, the columns the table reads, `episode_id` on every row, `REDSHIFT_*` from `~/.env` | `data/flc.parquet`, the day's extract |
...
- **`episode_id` is the opening tag** of the row's episode:
  `<sku_id>|<fc>|<day>T<hh>`, the shelf-hour the episode began. A row whose
...
table -- the columns the feature table reads, under the engine's names,
with your `episode_id` on every row -- and writes `data/flc.parquet`. It reads
...
negative or whose category is null.
...
| `data/` | `flc.parquet`, the day's extract: the trailing 45 days of the hourly table, the columns the feature table reads, overwritten every morning (a trailing 30-day rate needs 30 days of rows, and the previous episode's rate reaches a little further) | `download_flc.py` |
| `config.yaml` | the owner's config pruned to the keys the commands read, generated by the sync: the horizon cap, the category anchors, the five artifact paths, the table folder, the tier grid, the log folder. No switch: the folder prices whenever it is run | the owner's sync |
| `snapshots/` | `<day>T<hh>.parquet`, the twelve request fields per shelf at the top of the hour | you, hourly |
...
runs the table checks there on the files you send.
