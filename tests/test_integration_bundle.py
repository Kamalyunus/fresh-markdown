...
def test_the_pull_is_one_select_with_the_producers_id_and_no_credential_in_the_folder(tmp_path):
    ...
    r = _isolated(folder, "-c", (
        ...
        "for col in ('sku_id', 'hour_of_day', 'starting_inventory', 'units_sold', 'total_discount', 'category'):\n"
        "    assert col in q, col\n"
        ...
        "conn = Conn()\n"
        "pd.read_sql = lambda sql, c: pd.DataFrame({'sku_id': [1, 2], 'date': ['2026-08-27', '2026-08-28']})\n"
