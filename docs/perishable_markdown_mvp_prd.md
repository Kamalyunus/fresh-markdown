...
`reference_r` **must be a number, not null.** This step runs first, so on a fresh clone there is no `r_lookup.json` to borrow from; with it null the step refuses — correctly, rather than inventing a value — but the documented run order then fails before it starts, which is exactly what happened the first time it was run. The default **0.42** is the fitted global measured on the production extract, a documented constant in the same way `fallback_mean: -1.00` is. Pinning it also makes the bracket reproducible rather than dependent on whether a previous run left an artifact behind. `null` remains accepted and means "use the fitted global from `r_lookup.json` if one exists", for a re-run in the old order.
