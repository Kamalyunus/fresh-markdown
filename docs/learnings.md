...
- **The pricing host gets a folder, not a repository** (owner, 09-19).
  ...
  to price. `integration/` is that folder: GENERATED by
  `tools.build_integration` as the exact import closure of the four entry
  points, package layout kept so no import is rewritten, the config, the
  pinned requirements, the examples, the handover page and a README that
  is the short handoff. Generated, never edited: a second copy of a
