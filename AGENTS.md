...
- One package per responsibility: `engine/` prices and learns, `events/`
  ...
  the producers' id rule), `tools/` is out of review scope; `integration/` is GENERATED by `ops.integration` (three root commands over `src/`, Lane B's exact import closure, standalone for the pricing host; `seal` and `advance` sync the artifacts into it), never edited — its test pins it to a fresh build. A new module goes where
