...
| `exploration.delta_min_log_bias` | `tune` from `backtest.fidelity` (largest of MAE@W, `by_category` rms log, gate half-width) — null = no floor, the fixture never ships a number | `config mirrors reports` |
