...
- One package per responsibility: `engine/` prices and learns, `events/`
  ...
  the producers' id rule), `tools/` is out of review scope; `integration/` is GENERATED by `tools.build_integration` (Lane B's exact import closure, standalone for the pricing host, its README the handoff), never edited — its test pins it to a fresh build. A new module goes where
