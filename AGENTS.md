...
1b. **Drive the chain with `python3 -m ops.advance`, never by hand-running
   the step list** — it calls `ops.bootstrap_loop` (which iterates steps 3b–5b
   to the fixed point), re-runs after a paste only what read it (a W paste
   turns the loop with `--check-only`; the rest re-derive a report or
   nothing), and retrains only when the model is absent or `--retrain` is
   given. Re-running the step list retrains the baseline (rule 1). (§9.2)
...
3. **A boundary solution is not an estimate** — a fit pinned at a search
   ...
   `r_lookup.at_bound`, `calibration.pinned_cells` (every pin, a PARENT
   category's too) flag the others; `status` READS them all. (§5.5, §5.6, §9.3)
...
- Never re-derive logic that has one home. The homes, and what each
  ...
  - the finiteness test and the count checks with the horizon bound —
...
`pip install -r requirements.txt`, then `python3 -m pytest tests/`. All
commands run from the repo root — `config.yaml` paths are relative to it; a
module run from elsewhere silently reads/writes the wrong artifacts.
...
| --- | --- | --- | --- |
| data | `fit.download_flc` over `split.train_start` → the hold-out's end, when no extract is on disk; after launch the cron's `--feed` run refreshes it through yesterday whenever the factor schedule falls behind the week being priced (`REDSHIFT_*` on the cron host), then re-fits and re-seals | none | — |
...
| tune | `tune --apply` — each check is asked once the report(s) it reads exist (`tune.READS`), so what the backtest and thresholds measured is pasted BEFORE the first shadow, which prices on the final values and stands — then `ops.bootstrap_loop --check-only` / `evaluate.backtest` / `derive_thresholds` / shadow as the moved keys demand (`config_keys.stale_keys`; `READ_BY` routes unpasted keys to the one report or fit that reads them), until nothing is left to paste. A MEASURED value a report ran and still could not derive is a STOP naming that report, never an owner decision; then `ops.seal --reason config` (or `libraries`) whenever the environment moved since the last seal | `rho`, `calibration_gate_band`, `information_increment`, `delta_min_log_bias`, `scrap/margin_deterioration_pct` (the 3σ trailing floor), `max_mean_step` (inside its price-consequence gate); never `calibration_fit_trailing_weeks`, the owner's | the process, from the report that derives each |
...
`advance` exits 1): what ran per phase, every value the process changed
with before, after, why and source, the config in force, status, and what
...
one step per invocation; and it stops on every SET BY OWNER null with the
evidence. Its daily lane walks tau and stops at `daily.update --apply`,
which it never runs. Read its stop before doing anything by hand.
...
| `non_priceable` | `negative_window` | `window_too_long` | `opens_empty`
| `outcome_unknown` | `final_hour_restock`) flag what the solver cannot
price — nothing economic is dropped (rule 14). Resolve via `prepare_data.population(d, cfg, which)`:
...
**`config.yaml` ships the OWNER's production readings** (their `ops.advance` run of 2026-09-06: `rho` 0.6364, `information_increment` 0.237, `tau_initial` 348.93, the 16-category + `_default` `delta_min_log_bias` map, scrap/margin stops 0.3217/0.0614) **and their postures** (`max_std_shrink` 0.10 — conservative launch; `max_mean_step` 0.796, pasted inside its gate; both guardrail series smoothed over 7 days; `cold_start_shift_std` 0.5; `delta_min_bias_multiple` 1 — the forced-move gap, theirs to raise from shadow's sweep since a short move tilts a level error into ε, design 5.8) as the defaults — the table in design §12 is the record; a local run on the
...
| `information_increment`, `calibration_gate_band` (W, `calibration_fit_trailing_weeks`, is the OWNER's: the sweep advises, nothing pastes it) | the REPORT that derives each (`tune` names it) | `config mirrors reports` — these cannot be null, so the check is the only thing between a pulled repo and a number from another extract |
...
| `exploration.delta_min_log_bias` | `tune` from `backtest.fidelity`, PER CATEGORY as a one-line mapping (own log ratio floored by MAE@W and the gate half-width; `_default` for unseen categories) — null = no floor; a block-style mapping (a YAML re-dump) is REFUSED by the paste, never overwritten | `config mirrors reports` |
...
- One package per responsibility: `engine/` prices and learns, `events/`
  ...
  the producers' id rule), `tools/` is out of review scope; `integration/` is the pricing host's standalone folder, maintained IN PLACE: three commands, `price_hour.py`, `download_flc.py` and `build_features.py`, over `pricing/`, the folder's OWN code (fourteen modules written for the exploit-only, STATELESS hour — a row priced from itself: `episode_id` is the opening tag, `discount` the price in force piped back, features from the opening day's table, no store read — and the morning pull and table from the day's extract with the producers' ids as given, not copies of the repository's), a config pruned to what they read, and the five artifacts that `seal` and `advance` sync in — nothing in it fits, seals or checks; an engine fix is ported to `pricing/` by hand, and `test_end_to_end` prices the same hour through both and refuses a differing decision field. A new module goes where
