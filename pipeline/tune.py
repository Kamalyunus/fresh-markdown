...
def stale_keys(report, moved):
    ...
    readers (pipeline.advance and pipeline.status) use. The backtest's
...
RERUN_STEPS = {
    ...
    "shadow": ("shadow's own inputs moved -- re-run pipeline.shadow so its "
    ...
    "thresholds+shadow": ("a learning rail moved: re-derive thresholds AND "
                          "re-run pipeline.shadow (both read it)"),
    "calibration": (
