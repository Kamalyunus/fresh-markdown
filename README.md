...
| `integration/` | 9, App. A | The standalone pricing folder for engineering's host: the hourly command, the morning table it joins on, and what the two need, maintained IN PLACE (the owner's call: minimal, and no refactor of the repository for it). `price_hour.py` and `build_features.py` at its root chdir into the folder and run their module from `src/` — one file per repository module, each TRIMMED to the functions the two commands call (a function-level trace over entries, continuations, faults, the pool and two mornings; the learning lane's posterior update, the outcome side of the store, the pairing, the seal and the training-time split are gone; what a fixture cannot reach but a live hour can stays), `MANIFEST.json` saying per file what was trimmed. `ops.seal` and `ops.advance` call `integration.sync`, which copies the five artifacts the hour opens, the extract the first morning seeds from and the config pruned to the keys the commands read (`KEEP`, traced). `ops.integration --check` refuses a verbatim copy behind its source, a config behind a fresh prune, or a module the commands do not reach. Exploit-only by construction; the id is the producers'; the checks stay here on the files engineering sends. `tests/test_integration_bundle.py` pins all of it; `test_end_to_end` seals, syncs, builds the table and prices an hour from it. |
