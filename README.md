...
| `integration/` | 9, App. A | The standalone pricing folder for engineering's host, maintained IN PLACE (the owner's call: minimal, and no refactor of the repository for it). Three commands at its root (`price_hour.py` every hour, `build_features.py` every morning, `check_inputs.py` on their tables and on the hour's response), each chdir-ing into the folder and running its module from `src/` — the hourly path's modules, one file per repository module, most verbatim and a listed few curated (a re-export dropped, the applier split from the trainer, exploit-only fixed on), `MANIFEST.json` saying which and why. `ops.integration --check` refuses a verbatim copy that fell behind its source; `ops.seal` and `ops.advance` call `integration.sync` so the artifacts, the posterior, the extract and the config reach it without a hand copy. The episode id is the producers', nothing there assigns one, and the learning lane is deliberately absent. `tests/test_integration_bundle.py` pins the copies, the closure and the commands; `test_end_to_end` seals, syncs and prices an hour from it. |
