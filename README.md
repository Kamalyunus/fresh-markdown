...
| `integration/` | 9, App. A | The standalone pricing folder for engineering's host: the hourly command and what it needs, nothing else, maintained IN PLACE (the owner's call: minimal, and no refactor of the repository for it). `price_hour.py` at its root chdirs into the folder and runs `ops.price_hour` from `src/` — the hourly path's modules, one file per repository module, most verbatim and a listed few curated (a re-export dropped, the applier split from the trainer, the loader's one gate, exploit-only fixed on), `MANIFEST.json` saying which and why. Nothing in it generates an artifact or a feature table: `ops.seal` and `ops.advance` (the morning `--feed` run included) call `integration.sync`, which copies the five artifacts the hour opens, the day's feature table and the config pruned to the keys the hour reads (`KEEP`). `ops.integration --check` refuses a copy behind its source, a config behind a fresh prune, or a module the command does not reach. The checks stay here (`ops.check_inputs`, on the files engineering sends); the id is the producers'; the learning lane is absent. `tests/test_integration_bundle.py` pins all of it; `test_end_to_end` builds the table, seals, syncs and prices an hour from it. |
