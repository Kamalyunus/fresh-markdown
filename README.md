...
| `integration/` | 9, App. A | GENERATED by `ops.integration`, never edited: the standalone pricing folder for engineering's host. Three commands at its root (`price_hour.py` every hour, `build_features.py` every morning, `check_inputs.py` on their tables and on the hour's response), each chdir-ing into the folder and running its module from `src/` — the exact import closure of the three, copied verbatim with the package layout kept — the applier, never the trainer or a fitter, and no re-export: the repo keeps none — plus `config.yaml`, the pinned requirements, `examples/`, the handover page and its own `README.md` as the short handoff (`docs/integration_README.md`); `MANIFEST.json` digests every generated file. `ops.seal` and `ops.advance` call `integration.sync` so the sealed artifacts, the posterior, the extract and the config reach `artifacts/` and `data/` without a hand copy. The episode id is the producers' (nothing here assigns one) and the learning lane is deliberately absent. `tests/test_integration_bundle.py` pins the folder to a fresh build and runs it from elsewhere with the repository off the path; `test_end_to_end` seals, syncs and prices an hour from it. |
