...
and only `daily.update --resume-exploration` clears it. Assurance is its
own step (`daily.assurance`); status reads its report directly.
...
def apply_stop_conditions(stop, posterior, since):
    """Record a fired stop in the posterior store so engine.decide stops
    ...
    the suspension standing until `daily.update --resume-exploration`.
