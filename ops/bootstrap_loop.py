"""ops.bootstrap_loop -- the pipeline, with its loop driven to convergence.
...
retraining, which is what a config paste needs (see `ops.tune`); nothing
...
def settle(cfg, max_turns):
    ...
    for turn in range(1, max_turns + 1):
        ...
        step(f"loop turn {turn}: 4 prior (fast)",
             ["fit.estimate_prior", "--input", PREPARED, "--fast"])
        step(f"loop turn {turn}: 5 dispersion",
             ["fit.fit_dispersion", "--input", PREPARED])
        ...
        if rc:
            raise SystemExit(
                ...
                "and re-run `ops.bootstrap_loop --check-only`.")
        ...
        if block.get("converged"):
            ...
            step("prior: full re-run for the artifact",
                 ["fit.estimate_prior", "--input", PREPARED])
...
def main():
    ...
    if not args.check_only:
        ...
        step("step 1: prepare_data",
             ["fit.prepare_data", "--input", args.input,
              "--out", PREPARED])
        ...
        step("step 3: train_baseline", ["fit.train_baseline",
                                        "--input", PREPARED])
    ...
    if step("where this run stands: status", ["ops.status"], fatal=False):
...
Bootstrap complete -- loop settled in {turns} turn(s). Next:
  ...
  4. python3 -m ops.status -- "not run" is not a passing check.""")
    return 0
