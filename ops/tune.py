...
`FINDINGS` is the order the report lists them in. The key registry -- who
pastes what, who reads what, what a move re-runs -- is ops.config_keys.
"""
...
from common.config import artifact_mirror_drift, config_get, load_config
from common.guardrail import verdict_is_blocking, verdict_is_insufficient
from common.io import read_json, write_json
from common.paths import DECISIONS
# moved to ops.config_keys; the names stay here for callers
from ops.config_keys import (KEYS, MEASURED_KEYS, DERIVED_IN, RERUN,             # noqa: F401
                             RERUN_ORDER, INVALIDATES, RERUN_STEPS, ROUTED_REPORTS,
                             READ_BY, INERT_PREFIXES, _class_of, rerun_for,
                             rerun_classes, stale_keys, set_scalar,
                             tau_provenance_error)

PASTE, OWNER, INFO, BLOCK = "PASTE", "OWNER", "INFO", "BLOCK"
OK, ACT = "OK", "ACT"

# what every finding reads: the three reports and the two artifacts
Reports = namedtuple("Reports", "backtest shadow thresholds rho calibration",
                     defaults=(None,) * 5)
...
def _block_one_model(cfg, rep):
    """One model behind every report (hard rule 1)."""
    versions = {}
    for name, r in (("backtest", rep.backtest), ("shadow", rep.shadow)):
        v = ((r or {}).get("artifact_versions") or {}).get(
            "baseline_model_version")
        if v:
            versions[name] = v
    bundle = (rep.calibration or {}).get("provenance", {}).get("bundle")
    if len(set(versions.values())) > 1:
        return [_finding("reports agree on one model", BLOCK, ACT,
    ...
    if versions and bundle and set(versions.values()) != {bundle}:
        return [_finding("reports match the artifacts", BLOCK, ACT,
                         versions, bundle,
...
def _block_convergence(cfg, rep):
    """The calibration <-> dispersion fixed point settled, and still holds."""
    conv = (rep.calibration or {}).get("convergence")
    if not conv:
        return [_finding("calibration convergence", BLOCK, ACT,
...
def _block_split_sizing(cfg, rep):
    """The split obeys its own sizing rule (design 6): calib >= 2W, or the
    ...
    if calib_weeks < 2 * w:
        return [_finding("data.split", BLOCK, ACT,
...
def _block_exclusion_gap(cfg, rep):
    """No graded window straddles the exclusion gap."""
    out = []
    s = cfg["data"]["split"]
...
BLOCKS = (_block_one_model, _block_convergence, _block_split_sizing,
          _block_exclusion_gap)


# ------------------------------------------------------------------- PASTE
# MEASURED values the pipeline computed and config must mirror. Two of
# them carry a gate that downgrades the finding to OWNER with the reason.

def _paste_information_increment(cfg, rep):
    inc_rec = (rep.thresholds or {}).get("information_increment_recommendation") or {}
    rec = inc_rec.get("recommended")
    cur = config_get(cfg, ("learning", "information_increment"), default=None)
    if rec is not None:
        return [_finding(
    ...
    if str(inc_rec.get("verdict") or "").startswith("NOT RUN"):
...
def _paste_rho(cfg, rep):
    got = (rep.rho or {}).get("rho")
    cur = config_get(cfg, ("dispersion", "rho"), default=None)
    if got is None:
        return []
...
def _paste_tau_initial(cfg, rep):
    tau = ((rep.shadow or {}).get("tau_initial_derivation") or {}).get("tau_initial")
    cur = config_get(cfg, ("exploration", "tau_initial"), default=None)
    if tau is None:
        return []
    ...
    stale = tau_provenance_error(cfg, rep.backtest, rep.shadow)
    return [_finding(
...
def _paste_delta_min_bias(cfg, rep):
    ...
    cur = config_get(cfg, ("exploration", "delta_min_log_bias"), default=None)
    if floor is None:
        return []
...
def _rail_max_mean_step(cfg, rep):
    """The rail, gated on its measured price consequence: a paste inside
    the gate, an OWNER decision when the re-price exceeds it."""
    out = []
    backtest, thresholds = rep.backtest, rep.thresholds
    bs = (thresholds or {}).get("bounded_step_recommendation") or {}
    cur = config_get(cfg, ("learning", "max_mean_step"), default=None)
...
def _guardrail_stops(cfg, rep):
    """The guardrail stops: the floor IS the measurement -- 3-sigma of the
    series' own trailing noise. Setting them at it is what the report
    recommends; an unusable block is OWNER, an unmeasured one INFO."""
    out = []
    gr = (rep.thresholds or {}).get("guardrail_threshold_recommendation") or {}
    for name, block in gr.items():
        ...
        if not path or not path[0]:
            continue
        cur = config_get(cfg, path, default=None)
...
def _fit_window(cfg, rep):
    """The fit window W the rolling-origin sweep prefers, bounded by
    calib >= 2W (OWNER when the split cannot support it), with HYSTERESIS:
    W is the only paste that turns the calibration loop, and --check-only
    re-settling the loop moves the sweep's own scores -- so a strict
    argmin flips between near-tied windows and sends an agent into
    apply -> check-only forever. Switch only when the candidate beats the
    CURRENT window materially on the sweep's own metrics (the margins are
    config: tuning.w_switch_mae_ratio, tuning.w_switch_band_gain -- they
    decide whether a MEASURED value is pasted, so they are recorded in the
    snapshot like any knob); a near-tie holds the value in force."""
    sweep = _sweep_of(rep.backtest)
...
# ------------------------------------------------------------------- OWNER
# Tolerances data cannot decide; reported with evidence, never written by
# --apply.

def _owner_max_std_shrink(cfg, rep):
    """Which rail moves is a safety posture; the tool supplies both numbers."""
    bs = (rep.thresholds or {}).get("bounded_step_recommendation") or {}
    std0 = bs.get("median_launch_std")
    step = config_get(cfg, ("learning", "max_mean_step"), default=None)
    cur = config_get(cfg, ("learning", "max_std_shrink"), default=None)
    if not (std0 and step and float(std0) > 0):
        return []
...
def _owner_cold_start(cfg, rep):
    """The launch belief: an owner's risk posture, never pasted, graded by
    the backtest (DP at the belief, world at the prior mean)."""
    k = cfg["posterior"].get("cold_start_shift_std")
    pdl = (rep.backtest or {}).get("policy_deltas") or {}
    ie = pdl.get("intra_episode_deepening") or {}
    gap = pdl.get("policy_gap_like_for_like") or {}
    return [_finding(
...
def _info_calibration_cadence(cfg, rep):
    """The weekly cron is worth running only if it beats the frozen anchor
    on the hold-out. Measured, not assumed."""
    out = []
    cr = (rep.shadow or {}).get("calibration_regimes") or {}
...
def _info_learning_bottleneck(cfg, rep):
    ...
    if not (per_update and days and episodes):
        return []
...
def _info_uncalibrated_wins(cfg, rep):
    ...
    sweep = _sweep_of(rep.backtest)
    if not sweep.get("uncalibrated_beats_all_windows"):
        return []
    rec = sweep.get("recommended_fit_window")
...
# the report's order: the pastes, the rails, the owner postures, the readings
FINDINGS = (_paste_information_increment, _paste_rho, _paste_tau_initial,
            _paste_delta_min_bias, _rail_max_mean_step, _guardrail_stops,
            _paste_gate_band, _owner_max_std_shrink, _owner_cold_start,
...
def collect(cfg, root="reports", reports=None, artifacts=None):
    ...
    calibration = (artifacts.get("calibration")
                   or read_json(cfg["baseline_model"]["calibration_factor_path"]))
    rho_art = artifacts.get("rho") or read_json(cfg["dispersion"]["rho_path"])
    rep = Reports(backtest, shadow, thresholds, rho_art, calibration)
...
def apply(report, config_path="config.yaml", out_dir="artifacts", keys=None):
    ...
    log_path = os.path.join(out_dir, os.path.basename(DECISIONS))
