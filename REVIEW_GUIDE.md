...
| `events/store.py` (+ `events/contract.py`, the required fields and value checks it enforces) | Every emit under the store's lock with the streams' tails re-read first (`_locked`, `_consume`: two overlapping hourly runs cannot both price an hour — verify the refresh moves the offset past this store's own writes); append-only, durable writes, dedup on emit AND on load (a duplicated line is counted and loaded once; the id is registered after the append succeeds); malformed events **quarantine with the reason attached** rather than being dropped, and every stream's torn last line is closed on open; exactly three `adjustment_reason` values reconcile inventory; the two invariants live here and nowhere else — one decision per hour (`priced_hours`, a second one refused and counted — the gate that runs FIRST, because the decision id is now the shelf-hour and a re-priced hour trips both) and one outcome per decision (a second one refused on emit, skipped on load, counted; an outcome without `is_stockout` never lands) — `reset=True` empties only this store's own streams for a harness run and REFUSES the production store; a third stream holds the shelf-hours seen and NOT priced (`emit_rejection`: no price, never in `priced_hours`, never in the pairing, so `last_seen_by_shelf` spans both and a gap there is a missing hour rather than one of our refusals); and `episode_paths` holds the forecast a later hour of an episode is priced on |
...
## Tier 2 — review the state mutations (~900 lines; the only writes in production)
